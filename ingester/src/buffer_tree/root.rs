...
mod tests {
        ...
        (
            ...
            paste::paste! {
                ...
                async fn [<test_write_query_ $name>]() {
                    ...
                    let batches = buf
                        .query_exec(ARBITRARY_NAMESPACE_ID, ARBITRARY_TABLE_ID, vec![], None, $predicate)
                        .await
    ...
    test_write_query!(
        ...
        predicate =
            Some(Predicate::new().with_expr(col("region").eq(lit(&*PARTITION2_KEY.to_string())))), // TODO(savage): This needs double checking
        want = [
            "+----------+------+-------------------------------+",
            "| region   | temp | time                          |",
            "+----------+------+-------------------------------+",
